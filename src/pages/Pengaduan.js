...
export default class Pengaduan extends React.Component {
    ...
    getPengaduansAp = () => {
        let url = base_url + "/pengaduan/get/status/Approved"
        axios.get(url, this.headerConfig())
            .then(response => {
                this.setState({ pengaduansAp: response.data })
            })
            .catch(error => {
                if (error.response) {
    ...
    getPengaduansRe = () => {
        let url = base_url + "/pengaduan/get/status/Rejected"
        axios.get(url, this.headerConfig())
            .then(response => {
                this.setState({ pengaduansRe: response.data })
            })
            .catch(error => {
                if (error.response) {
                    if (error.response.status) {
    ...
    getPengaduansDo = () => {
        let url = base_url + "/pengaduan/get/status/Selesai"
        axios.get(url, this.headerConfig())
            .then(response => {
                this.setState({ pengaduansDo: response.data })
            })
            .catch(error => {
                if (error.response) {
                    if (error.response.status) {
    ...
    componentDidMount() {
        this.getPengaduansAp()
        this.getPengaduansRe()
        this.getPengaduansDo()
        this.getPengaduans()
        this.getTanggapans()
        this.initPetugas()
    }
        ...
        return (
            <div>
                ...
                <div className="container pb-4" >
                    ...
                    <h4 className="text-bold text-primary my-2" >Approved</h4>
                    <div className="row">
                        {this.state.pengaduansAp.map(item => (
                            <AduanList
                    ...
                    {/* <h4>Rejected</h4>
                    <div className="row">
                        {this.state.pengaduansRe.map(item => (
                            <AduanList
                                key={item.id_pengaduan}
                                tgl_pengaduan={item.createdAt}
                                nik={item.nik}
                    ...
                    <h4 className="text-bold text-success my-2">Selesai</h4>
                    <div className="row">
                        {this.state.pengaduansDo.map(item => (
                            <AduanList
                                key={item.id_pengaduan}
                                tgl_pengaduan={item.createdAt}
                                nik={item.nik}
                                ...
                                <form onSubmit={ev => this.savePengaduan(ev)}>
                                    <RadioGroup name="fruits" className="mb-2" >
                                        <div className="radio-button-background">
                                            <Radio value="Selesai" onChange={ev => this.setState({ status: ev.target.value })} className="radio-button" /> Selesai
                                            </div>
                                    </RadioGroup>
